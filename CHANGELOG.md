...
- The load order's section colors are one continuous ramp, taken from the list's own MO2 pane: burnt orange fading to black down the content half, then black deepening into blue through the rest. Before, the blue was a second, separate gradient that started at AI Improvements, which put a hard edge in the middle of the list. The sections also run flush against each other now, with no rule between them, so the ramp reads as one wash rather than as banded rows.
